  ...
  <div class="future-page">
    ...
    <div v-if="resultVisible" class="result-mask" role="dialog" aria-modal="true" aria-labelledby="result-title" @click.self="closeResult">
      <div class="result-dialog">
        ...
        <p v-if="resultCard?.imageKey" class="cos-upload-note">高清图片已同步到云端存储</p>
...
async function removeCard(card) {
  if (!window.confirm('确定删除这封信吗？删除后无法恢复。')) r…500 tokens truncated…lientError('EXPORT_FAILED', '图片生成失败')), 'image/png'))
}

async function uploadCardImage(card, blob) {
  if (!card?.id || !blob) return null
  const base = String(import.meta.env.VITE_API_BASE || '/api').replace(/\/+$/, '')
  const token = localStorage.getItem('token') || ''
  const response = await fetch(`${base}/future-cards/${encodeURIComponent(card.id)}/image`, {
    method: 'POST',
    credentials: 'include',
    headers: {
  ...
  if (!response.ok || body?.code !== 0) {
    throw clientError(body?.errorCode || 'IMAGE_UPLOAD_FAILED', body?.message || '云端图片保存失败')
  }
  return body.data?.card || null
}
...
.cos-upload-note{margin:-8px 0 16px;color:#176b52;font-size:11px}
</style>
