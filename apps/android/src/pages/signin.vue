...
async function handleLoginStrict(name, pass) {
  // Prefer /auth/login and only fall back when endpoint is unavailable
  let loginResp = null
  try {
    loginResp = await request('/auth/login', 'POST', { username: name, password: pass })
  } catch (err) {
    if (!isEndpointUnavailable(err)) throw err
  }

  if (loginResp) {
    const ok = normalizeOk(loginResp)
    if (ok) {
      persistLogin(loginResp)
      return
    }
    if (!isEndpointUnavailable(loginResp)) {
      const why = normalizeWhy(loginResp)
      if (why === 'USER_NOT_FOUND') throw new Error('Account not found')
      if (why === 'BAD_PASSWORD') throw new Error('Wrong password')
      throw new Error(normalizeMsg(loginResp) || 'Login failed')
    }
  }

  const exists = await safeCheckUserExists(name)
  if (exists === false) throw new Error('Account not found')
  ...
  try {
    ...
    if (!ok) {
      ...
      if (why === 'USER_NOT_FOUND') throw new Error('Account not found')
      if (why === 'BAD_PASSWORD') throw new Error('Wrong password')
      throw new Error(normalizeMsg(resp) || 'Login failed')
    }
    persistLogin(resp)
  } catch (e) {
    throw new Error(normalizeMsg(e) || 'Login API unavailable or server error')
  }
}
...
function isEndpointUnavailable(x) {
  const status = Number(x?.statusCode || x?.status || x?.response?.status || 0)
  if ([0, 404, 405, 501, 502, 503].includes(status)) return true
  const msg = String(x?.message || x?.data?.message || '').toLowerCase()
  return msg.includes('network_error') || msg.includes('timeout') || msg.includes('failed to fetch')
}
function normalizeOk(resp) {
  const d = unwrapRespData(resp)
  if (typeof d?.code === 'number') return d.code === 0
  if (typeof resp?.code === 'number') return resp.code === 0
  if (resp?.status && (resp.status < 200 || resp.status >= 300)) return false
  return !!(d?.token || d?.userInfo || d?.data?.token || d?.data?.userInfo)
}
function normalizeMsg(resp) {
  const d = unwrapRespData(resp)
  const msg =
    d?.message || d?.msg || d?.error || d?.detail ||
    (typeof d === 'string' ? d : '') ||
    resp?.statusText || ''
  if (msg) return String(msg)
  if (resp?.statusCode || resp?.status) return `${resp.statusCode || resp.status}`
  return ''
}
function normalizeWhy(resp) {
  ...
  if (code === 1001 || /user.*not.*exist/.test(msg)) return 'USER_NOT_FOUND'
  if (code === 1002 || /password/.test(msg)) return 'BAD_PASSWORD'
  return ''
}
